...
Event_leaveCnode.validate = function(structure, agent) {
  ...
  if (!(res = server.models.Event.validate_key_format(server.models.Event_leaveCnode.formats, structure)).status) {
    return res;
  }
  ...
  if (!(res = server.models.Event.validate_cnode_has_agent(res.cnode, agent)).status) {
    return res;
  }
  return res;
};
