...
server.models.Event_moveToRoom.validate = function(structure, agent) {

  if (!(res = server.models.Event.validate_key_format(server.models.Event_moveToRoom.formats, structure)).status) {
    return res;
  }
  if (!(res = server.models.Event.validate_room_adjacent(agent.room, Room.get_room_by_id(structure.room_id))).status) {
    return res;
  }
  return res;
};
...
function Event_moveToRoom(socket, inputData) {
  ...
  if (!(res = server.models.Event_moveToRoom.validate(inputData, agent)).status) {
